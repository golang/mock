...
func main() {
	...
	if err := g.Generate(pkg, packageName, outputPackagePath, sourceHash); err != nil {
...
func sourceNotModified(source, dst string) (bool, string) {
	...
	rxHash := regexp.MustCompile(`// Hash: ([a-z0-9]+)`)
	matches := rxHash.FindSubmatch(dstContent)
	if len(matches) < 2 {
		// hash comment not found
		return false, hash
	}

	return hash == string(matches[1]), hash
}
...
func (g *generator) Generate(pkg *model.Package, pkgName, outputPackagePath, sourceHash string) error {
