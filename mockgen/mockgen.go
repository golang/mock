...
var (
	...
	sourcePackage   = flag.String("source_package", "", "Full package name for interface to be mocked. Required to support references to structs from same package.")
...
func main() {
	...
	g := new(generator)
	g.srcPackageName = pkg.Name
	if *source != "" {
		g.filename = *source
		g.srcPackage = *sourcePackage
	} else {
...
func typeNeedsToBeImported(typeName, sourcePackageName, destPackageName string) bool {
	firstLetterCode := typeName[0]
	return sourcePackageName != destPackageName && "A"[0] <= firstLetterCode && firstLetterCode <= "Z"[0]
}

func isInterfacePackageImportNecessary(interfaces []*model.Interface, sourcePackageName string, packageMap map[string]string) bool {
	for _, intf := range interfaces {
		for _, m := range intf.Methods {
			for _, p := range m.In {
				argType := p.Type.String(packageMap, *selfPackage)
				if typeNeedsToBeImported(argType, sourcePackageName, *packageOut) {
					return true
				}
			}
			for _, p := range m.Out {
				retType := p.Type.String(packageMap, *selfPackage)
				if typeNeedsToBeImported(retType, sourcePackageName, *packageOut) {
					return true
				}
			}
		}
...
func (g *generator) Generate(pkg *model.Package, pkgName string) error {
	...
	if isInterfacePackageImportNecessary(pkg.Interfaces, g.srcPackageName, g.packageMap) {
		if g.srcPackage == "" {
			log.Println("WARNING: References to exported types from source file detected. " +
				"Use `-source_package` flag to ensure proper import in generated mock package.")
		}
		im[g.srcPackage] = true
	}
...
func (g *generator) GenerateMockMethod(mockType string, m *model.Method, pkgOverride string) error {
	...
	for i, p := range m.Out {
		retType := p.Type.String(g.packageMap, pkgOverride)
		if typeNeedsToBeImported(retType, g.srcPackageName, *packageOut) {
			retType = fmt.Sprintf("%s.%s", g.srcPackageName, retType)
		}
		rets[i] = retType
	}
...
func (g *generator) getArgTypes(m *model.Method, pkgOverride string) []string {
	...
	for i, p := range m.In {
		argType := p.Type.String(g.packageMap, pkgOverride)
		if typeNeedsToBeImported(argType, g.srcPackageName, *packageOut) {
			argType = fmt.Sprintf("%s.%s", g.srcPackageName, argType)
		}
		argTypes[i] = argType
	}
	if m.Variadic != nil {
